...
full_returns <- read.csv("trimmed_returns_raw.csv")
...
```{r}
full_returns <- select(full_returns, permno = PERMNO, date = date, return = RET, ticker = TICKER, delisting_return = DLRET) %>%
  filter(ticker %in% masterIndex$TICKER) %>% # only need tickers for which we have filings to compare with
...
returns_calculator <- function(the_ticker, start_date, interval_length = 3, interval_type = "day"){
  end_date <- start_date %>%
  ...
  date_sequence <- seq(ymd(start_date), end_date, by = "days") # see above comment
...
```{r}
returns_3_days <- map2_df(masterIndex$TICKER, masterIndex$DATE_FILED, returns_calculator)
returns_7_days <- map2_df(masterIndex$TICKER, masterIndex$DATE_FILED, returns_calculator, interval_length = 7)
returns_14_days <- map2_df(masterIndex$TICKER, masterIndex$DATE_FILED, returns_calculator, interval_length = 14)
...
Treat the market cap data.

```{r}
full_cap <- mutate(full_cap, year = year(ymd(date)), month = month(ymd(date)), day = day(ymd(date)))
```

Join cap and returns data to calculate market cap.

```{r}
full_returns <- left_join(full_returns, full_cap, by = c("permno", "year", "month"))
```


Given a ticker and a date from the master Index, devise a formula to calculate holding period returns 
